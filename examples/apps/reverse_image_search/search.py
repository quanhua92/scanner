...
def make_op_graph(input):
    ...
    caffe = db.ops.FasterRCNN(
        ...
        args=caffe_args,
        device=DeviceType.GPU)
    frcnn_output = db.ops.FasterRCNNOutput(inputs=[(
        caffe, ["cls_prob", "rois", "fc7"])])
    return frcnn_output
...
def build_index():
    ...
    if not db.has_table('example_frcnn'):
        ...
        [example], _ = db.ingest_videos(
            [('example', '/bigdata/videos/movies/anewhope.m4v')], force=True)
...
def query(path, fvec_index, bbox_index):
    ...
    # TODO(): fix this silly hack when new_table properly
    # supports force=True
    ...
    [query_output_table] = db.run(
        db.sampler().all([(q_t, 'query_output')]),
...
def visualize(results):
    ...
    for i, (frame_index, bbox, k) in enumerate(to_vis):
        ...
        cv2.rectangle(frame, (int(bbox.x1), int(bbox.y1)),
                      (int(bbox.x2), int(bbox.y2)), (255, 0, 0), 3)
...
def main():
    path = sys.argv[1] if len(
        sys.argv) > 1 else '{}/query.jpg'.format(STATIC_DIR)
